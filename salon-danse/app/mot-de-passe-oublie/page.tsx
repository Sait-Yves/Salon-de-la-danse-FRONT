...
export default async function MotDePasseOubliePage({ searchParams }: { searchParams: Promise<{ email?: string; token?: string }> }) {
    ...
    <div className="surface-grain flex min-h-[calc(100vh-5rem)] items-center justify-center p-4">
      <div className="animate-fade-up w-full max-w-md rounded-3xl border border-[#7A291E]/10 bg-white p-8 shadow-[0_10px_40px_-12px_rgba(62,21,15,0.25)]">
        <div className="mb-6 text-center">
          <h1 className="font-['Montserrat'] text-2xl font-extrabold text-[#333333]">Mot de passe oublié</h1>
        </div>
        <ResetForms email={email ?? ""} token={token ?? ""} />
      </div>
    </div>
  );
}
