...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ mode?: string; expired?: string; reset?: string; next?: string }> }) {
  const user = await getMe();
  if (user) redirect(user.role === "admin" ? "/admin" : "/dashboard");
  const { mode, expired, reset, next } = await searchParams;
  return (
    <div className="surface-grain flex min-h-[calc(100vh-5rem)] items-center justify-center p-4">
      <div className="animate-fade-up w-full max-w-md rounded-3xl border border-[#7A291E]/10 bg-white p-8 shadow-[0_10px_40px_-12px_rgba(62,21,15,0.25)]">
        ...
        <LoginForms initialMode={mode === "password" || next ? "password" : "code"} next={next} expired={expired === "1"} reset={reset === "1"} />
      </div>
    </div>
  );
}
